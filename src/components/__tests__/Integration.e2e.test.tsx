...
jest.mock('../MaterialSelectionModal', () => ({
  MaterialSelectionModal: ({ isOpen, onClose, onSelect, materials }: any) => {
    if (!isOpen) return null;
    return (
      <div data-testid="material-selection-modal">
        {materials && materials.length > 0 && (
          <button
            onClick={() => {
              onSelect(materials[0].id);
              onClose();
            }}
            data-testid="select-first-material-btn"
          >
            Selecionar {materials[0].name}
          </button>
        )}
      </div>
    );
...
describe('Integração E2E - Fluxo Completo de Produção', () => {
  ...
  describe('Fluxo Completo: Material → Orçamento → PDF', () => {
    it('deve criar material, adicionar ao orçamento e gerar PDF', async () => {
      ...
      const addItemBtn = screen.getByText(/Adicionar Material/i);
      ...
      const selectMaterialBtn = screen.getByTestId('select-first-material-btn');
    ...
    it('deve calcular custos corretamente no fluxo completo', async () => {
      ...
      fireEvent.click(screen.getByTestId('select-first-material-btn'));
  ...
  describe('Fluxo com Frete', () => {
    it('deve incluir frete no cálculo final', async () => {
      ...
      // Habilitar frete
      const freightCheckbox = screen.getByRole('checkbox', { name: /Incluir frete/i });
      fireEvent.click(freightCheckbox);

      // Adicionar valor de frete
      const freightInput = screen.getByPlaceholderText(/Custo do Frete/i);
  ...
  describe('Validação de Integridade End-to-End', () => {
    it('deve manter consistência de dados em todo o fluxo', async () => {
      ...
      expect(quotes[0].id).toMatch(/^ORD-/);
      expect(quotes[0].createdAt).toBeTruthy();
    });
  });
