...
export const generateQuotePDF = (quote: Quote, materials: Material[], settings: AppSettings, calculated: CalculatedCosts): Blob | string => {
  ...
  doc.text(`Custo Material: R$ ${calculated.materialCost.toFixed(2)}`, 15, summaryY);
  summaryY += 7;
  ...
  if (quote.isFreightEnabled && quote.freightCost > 0) {
    doc.text(`Frete: R$ ${quote.freightCost.toFixed(2)}`, 15, summaryY);
    summaryY += 7;
  }
  
  doc.text(`Peso Total Geral: ${calculated.totalWeight.toFixed(2)} kg`, 15, summaryY);
  summaryY += 7;
